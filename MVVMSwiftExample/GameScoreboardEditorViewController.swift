...
class GameScoreboardEditorViewController: UIViewController {
  ...
  fileprivate func fillUI(){
    ...
    self.homePlayer1View.viewModel = viewModel.homePlayers[0]
    self.homePlayer2View.viewModel = viewModel.homePlayers[1]
    self.homePlayer3View.viewModel = viewModel.homePlayers[2]
    
    self.awayPlayer1View.viewModel = viewModel.awayPlayers[0]
    self.awayPlayer2View.viewModel = viewModel.awayPlayers[1]
    self.awayPlayer3View.viewModel = viewModel.awayPlayers[2]
    viewModel.score.bind {[unowned self] in self.scoreLabel.text = $0}
    viewModel.time.bind {[unowned self] in self.timeLabel.text = $0}
    viewModel.isPaused.bind {[unowned self] in  let title: String = $0 ? "Start" : "Pause"
