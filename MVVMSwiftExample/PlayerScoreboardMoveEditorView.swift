...
class PlayerScoreboardMoveEditorView: UIView {
  ...
  fileprivate func fillUI() {
   ...
   viewModel.onePointMoveCount.bind {[unowned self] in self.onePointCountLabel.text = $0}
    viewModel.twoPointMoveCount.bind{ [unowned self] in self.twoPointCountLabel.text = $0}
    viewModel.assistMoveCount.bind {[unowned self] in self.assistCountLabel.text = $0 }
    viewModel.reboundMoveCount.bind { [unowned self] in self.reboundCountLabel.text = $0}
    viewModel.foulMoveCount.bind {[unowned self] in self.foulCountLabel.text = $0}
  }
